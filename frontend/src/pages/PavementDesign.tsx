  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Layer configuration · Traffic parameters</p>
      </div>
          ...
          <div className="space-y-3">
            {layers.map((layer, i) => (
              <div key={i} className="flex gap-2 items-end">
                ...
                <div className="w-28">
                  <label className="block text-xs text-muted-foreground mb-1">Thickness (mm)</label>
                ...
                <button onClick={() => removeLayer(i)} className="p-2 text-muted-foreground hover:text-danger transition-colors">
      ...
      <div className="card">
        ...
        <p className="text-xs text-muted-foreground mb-3">Fatigue: Nf = K1 × (1/εt)^K2 × (1/E)^K3 &nbsp;·&nbsp; Rutting: Nr = K4 × (1/εv)^K5</p>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div>
            <label className="block text-xs text-muted-foreground mb-1">K1 (fatigue factor)</label>
            <input type="number" step="1e15" value={K1} onChange={e => setK1(Number(e.target.value))} className="input-field" min={0} />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">K2 (strain exponent)</label>
            <input type="number" step={0.01} value={K2} onChange={e => setK2(Number(e.target.value))} className="input-field" min={0} />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">K3 (modulus exponent)</label>
            <input type="number" step={0.01} value={K3} onChange={e => setK3(Number(e.target.value))} className="input-field" min={0} />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">K4 (rutting factor)</label>
            <input type="number" step="1e-7" value={K4} onChange={e => setK4(Number(e.target.value))} className="input-field" min={0} />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">K5 (rutting exponent)</label>
      ...
      <div className="card text-center">
        <p className="text-sm text-muted-foreground mb-4">Design params are sent to the Life Prediction section where you'll also enter strain values.</p>
