...
function StrainTable({ gauges }: { gauges: any[] }) {
  return (
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <MetricCard title="εt (p95)" value={data?.has_horizontal && data?.eps_t ? `${data.eps_t} µε` : '—'} subtitle="Horizontal tensile" color="accent" icon="M7 11l5-5m0 0l5 5m-5-5v12" />
        <MetricCard title="εv (p95)" value={data?.has_vertical && data?.eps_v ? `${data.eps_v} µε` : '—'} subtitle="Vertical compressive" color="secondary" icon="M13 5l5 5-5 5M5 5l5 5-5 5" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {data?.plot_json2 && <InteractivePlot plotJson={data.plot_json2} title="Collective Strain Distribution" />}
      </div>

      <div className="space-y-4">
        {(data?.has_horizontal || data?.has_vertical) ? (
          <>
            {data?.has_horizontal && hGauges.length > 0 && (
              <div className="card">
                <h3 className="card-title">Horizontal Strain Gauges</h3>
                <StrainTable gauges={hGauges} />
              </div>
            )}
            {data?.has_vertical && vGauges.length > 0 && (
              <div className="card">
                <h3 className="card-title">Vertical Strain Gauges</h3>
                <StrainTable gauges={vGauges} />
              </div>
            )}
          </>
          ...
          <div className="card">
            ...
            <StrainTable gauges={data.per_gauge} />
          </div>
        ) : (
          <div className="card text-gray-400 text-center py-8 border-dashed border-2 border-gray-200">
            ...
            <p className="text-xs mt-1">Upload VER and/or HOR files to see strain data</p>
          </div>
        )}
      </div>
    </div>
