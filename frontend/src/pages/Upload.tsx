  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Upload GEOTRAN .xls DAQ files — specify each file as Vertical or Horizontal</p>
      </div>

      <div
        className={`card border-2 border-dashed text-center ${dragging ? 'border-secondary bg-blue-50' : 'border-border'} transition-colors`}
      ...
      >
        ...
        <p className="text-muted-foreground mb-2">Drag & drop GEOTRAN .xls files here</p>
        ...
        <div className="card">
          ...
          <div className="mt-4 flex gap-3">
            ...
            <button onClick={() => { setFiles([]); setDone(false); setError('') }}
              className="btn-secondary bg-muted text-muted-foreground hover:bg-muted">Clear</button>
          </div>
        </div>
      )}
      ...
      {done && (
        <div className="card border-green-200">
          ...
          <p className="text-sm text-muted-foreground mb-3">{files.length} file(s) saved.</p>
          <div className="space-y-1 mb-3">
            <p className="text-xs text-muted-foreground">VER files: {verFiles.length} ({verFiles.map(f => f.file.name).join(', ')})</p>
            <p className="text-xs text-muted-foreground">HOR files: {horFiles.length} ({horFiles.map(f => f.file.name).join(', ')})</p>
          </div>
